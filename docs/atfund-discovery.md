...
This app uses the lookup hostname (currently: the user’s PDS hostname from OAuth token audience) when evaluating allowlists.
...
After sign-in, the app reads the PDS hostname from the OAuth token audience, resolves its DID via `_atproto`, and shows a **Your host** section when disclosure metadata is found.
