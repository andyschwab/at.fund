...
export type PdsHostFunding = {
  pdsHostname: string
  stewardDid: string
  /** Optional donation/support links. When omitted/empty, the steward is known but not accepting contributions. */
  links?: FundLink[]
  /**
   * Host-scoped dependencies (from fund.at.dependencies without appliesToNsidPrefix),
   * expressed as DIDs or hostnames.
   */
...
/**
 * Loads fund.at.contribute (+ optional fund.at.disclosure meta, fund.at.dependencies)
 * for a steward DID, using restrictToDomains when present. Chooses the newest
 * effectiveDate contribute record that has links; same for disclosure meta.
 */
export async function fetchPdsHostFunding(
  ...
  const contributePicked = contributeValues.filter((v) =>
    allowlistedForDomain(v, pdsHostname),
  )
  const bestContribute = pickBestContributeRecord(contributePicked)
  const links = bestContribute ? readLinks(bestContribute) : undefined

  let disclosure: PdsHostFunding['disclosure']
  try {
    ...
    const discPicked = discValues.filter((v) =>
      allowlistedForDomain(v, pdsHostname),
    )
    const best = pickBestDisclosureRecord(discPicked)
    if (best) {
  ...
  try {
    ...
    for (const r of depListed.data.records ?? []) {
      ...
      if (!allowlistedForDomain(rec, pdsHostname)) continue
  ...
  // Disclosure is the “known-good” anchor. If it’s missing, treat this as no funding metadata.
